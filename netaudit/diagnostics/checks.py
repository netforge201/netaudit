...
def reachability_check(target: str, timeout: float = 2.0) -> CheckResult:
    ...
    if result.reachable and result.packet_loss_pct == 0:
        return CheckResult(
            "Reachability", CheckStatus.PASS, f"Host is reachable (avg {result.avg_ms} ms)"
        )
    if result.reachable:
        return CheckResult(
            "Reachability",
    ...
    return CheckResult("Reachability", CheckStatus.FAIL, result.error or "Host is not reachable")
...
def packet_loss_check(target: str, timeout: float = 2.0) -> CheckResult:
    ...
    if result.packet_loss_pct > 0:
        return CheckResult(
            "Packet loss", CheckStatus.WARN, f"{result.packet_loss_pct}% packet loss detected"
        )
...
def open_services_check(target: str, timeout: float = 1.0) -> CheckResult:
    ...
    if not open_ports:
        return CheckResult("Open services", CheckStatus.WARN, "No common services detected open")
